...
class Vehicle:
    ...
    def update_positions(self, vehicle: Any, x_coord: float, not_left_lane: bool, not_right_lane: bool,
        ...
        Args:
            ...
            not_left_lane (bool): current vehicle left lane position check   用于检查当前车辆是否不在最左或最右车道，避免越界
    ...
    def update_driving_params(self, surrounding: Dict[str, Any]) -> None:
        ...
        if surrounding['front'] is not None:
            ...
            #dist应该是指distance，这里是前车的位置减去自己的位置减去安全距离
            ...
            #onramp是指匝道，这里是匝道的长度加上匝道的偏移量减去自己的位置减去安全距离减去车辆长度
            front_v = self.local_v
        else:
            dist = self.road_length
            #在sumo中应该这么写
            # vehicle_id = "your_vehicle_id"  # 替换为需要查询的车辆ID
            # lane_id = traci.vehicle.getLaneID(vehicle_id)  # 获取车辆所在的车道ID
            # lane_length = traci.lane.getLength(lane_id)  # 获取该车道的长度
            front_v = self.local_v
